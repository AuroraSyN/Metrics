...
public class Max extends Metric implements Serializable {
	...
	private static final long serialVersionUID = 3257854268303095861L;
	...
	public static Max createFromChildMetrics(String name, String per, String childName, IMetricsElement source) {
		double max = 0; //will be initialized with first max (if any)
		IJavaElement element = null;
		IMetricsElement[] children = source.getChildren();
		for (int  i = 0; i < children.length; i++) {
			IMetricsElement next = children[i];
			Metric nm = next.getMetric(childName);
	...
	public static Max createFromMaxes(List values) {
		Max max = null;
		for (Iterator i = values.iterator(); i.hasNext();) {
			Max next = (Max)i.next();
